...
# Load Modules
#ys.path.append("../../MS models/fiora/fiora/")
...
def filter_CE_weight(df):
    ...
    print(f"Filtering spectra with ({num_ori}-{df.shape[0]})")
...
def filter_peaks(df):
    ...
    print("Removed ", sum(df["num_peak_matches"] < 2), "due to less than 2 peaks")
...
def train_new_model(continue_with_model=None):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience = 4, factor=0.1, mode = 'min')
...
if __name__ == '__main__':
    ...
    results = [{"validation": np.median(val_results), "test": np.median(test_results)}]
    print(results)
